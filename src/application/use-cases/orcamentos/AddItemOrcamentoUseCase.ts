...
export class AddItemOrcamentoUseCase {
  ...
  async execute(input: AddItemOrcamentoInput): Promise<AddItemOrcamentoOutput> {
    ...
    const itens = await this.itemOrcamentoRepository.findByOrcamentoId(input.orcamentoId);
    
    // Calcular novo subtotal
    const valorSubtotal = itens.reduce((acc, item) => acc + item.getSubtotal(), 0);
    const descontoTotal = itens.reduce((acc, item) => acc + item.descontoValor, 0);
    const descontoPercentual = valorSubtotal > 0 ? (descontoTotal / valorSubtotal) * 100 : 0;

    // Atualizar valores do orçamento
    orcamento.atualizarValores(valorSubtotal, descontoPercentual, descontoTotal);
