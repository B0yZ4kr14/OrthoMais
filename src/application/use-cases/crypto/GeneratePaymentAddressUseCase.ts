...
export class GeneratePaymentAddressUseCase {
  ...
  private async generateExchangeAddress(
    ...
    const qrCodeData = this.generateBIP21URI(dto.cryptocurrency, depositAddress, dto.amount, `Order-${dto.orderId}`);
  ...
  private async generateOfflineWalletAddress(
    ...
    const derivedAddress = this.mockDeriveAddress('xpub...', nextIndex, 'p2wpkh');
    const qrCodeData = this.generateBIP21URI(dto.cryptocurrency, derivedAddress, dto.amount, `Order-${dto.orderId}`);
   ...
   */
  private mockDeriveAddress(xpub: string, index: number, addressType: string): string {
...
export const generatePaymentAddressUseCase = new GeneratePaymentAddressUseCase();
