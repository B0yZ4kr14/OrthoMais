...
import { ToothStatus, ToothSurface } from '@/modules/pep/types/odontograma.types';
...
export class UpdateToothSurfaceUseCase {
  constructor(
    private readonly odontogramaRepository: IOdontogramaRepository
  ) {}

  async execute(input: UpdateToothSurfaceInput): Promise<UpdateToothSurfaceOutput> {
    // Validações de input
    ...
    const odontograma = await this.odontogramaRepository.findById(input.odontogramaId);
