...
import { contratoSchema, type Contrato } from '@/modules/contratos/types/contrato.types';
...
export function ContratoForm({ onSubmit, onCancel, initialData }: ContratoFormProps) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input id="patient_id" {...register('patient_id')} placeholder="ID do Paciente" />
        ...
        <div className="space-y-2">
          ...
          <Input id="numero_contrato" {...register('numero_contrato')} placeholder="Ex: CTR-2024-001" />
          {errors.numero_contrato && <p className="text-sm text-destructive">{errors.numero_contrato.message}</p>}
        </div>

        <div className="space-y-2 md:col-span-2">
          <Label htmlFor="titulo">Título *</Label>
          <Input id="titulo" {...register('titulo')} placeholder="Ex: Contrato de Tratamento Ortodôntico" />
        ...
        <div className="space-y-2">
          ...
          <Input id="orcamento_id" {...register('orcamento_id')} placeholder="ID do Orçamento (opcional)" />
        </div>

        <div className="space-y-2">
          <Label htmlFor="template_id">Template</Label>
          <Input id="template_id" {...register('template_id')} placeholder="ID do Template (opcional)" />
        </div>

        <div className="space-y-2">
          <Label htmlFor="valor_contrato">Valor do Contrato (R$) *</Label>
          <Input id="valor_contrato" type="number" step="0.01" {...register('valor_contrato', { valueAsNumber: true })} />
          {errors.valor_contrato && <p className="text-sm text-destructive">{errors.valor_contrato.message}</p>}
        </div>
        ...
        <div className="flex items-center space-x-2 md:col-span-2">
          <Checkbox
            ...
            onCheckedChange={(checked) => setValue('renovacao_automatica', checked as boolean)}
          />
        ...
        <div className="space-y-2 md:col-span-2">
          ...
          />
          {errors.conteudo_html && <p className="text-sm text-destructive">{errors.conteudo_html.message}</p>}
        </div>
      </div>
