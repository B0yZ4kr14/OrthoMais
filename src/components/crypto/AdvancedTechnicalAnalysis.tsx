...
export function AdvancedTechnicalAnalysis({ coinType }: TechnicalAnalysisProps) {
  ...
  const generateMockData = () => {
    const dataPoints = period === '24h' ? 24 : period === '7d' ? 168 : period === '30d' ? 720 : 365;
    const basePrice = coinType === 'BTC' ? 350000 : coinType === 'ETH' ? 18000 : 5.5;
    ...
    setIndicators({
      rsi: currentRSI,
      rsiSignal: currentRSI > 70 ? 'SOBRECOMPRA' : currentRSI < 30 ? 'SOBREVENDA' : 'NEUTRO',
      macd: currentMACD,
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <Badge 
                    variant={
                      indicators.rsiSignal === 'SOBRECOMPRA' ? 'destructive' :
                      indicators.rsiSignal === 'SOBREVENDA' ? 'success' : 'secondary'
                    }
                    className="mt-2"
                  >
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <Badge 
                    variant={indicators.macdSignal === 'ALTA' ? 'success' : 'destructive'}
                    className="mt-2"
                  >
                      ...
                      return (
                        <div className="bg-card border rounded-lg p-3 shadow-lg">
                          <p className="text-xs text-muted-foreground mb-2">
                            {format(new Date(payload[0].payload.timestamp), "dd/MM/yyyy HH:mm", { locale: ptBR })}
                          </p>
                          <p className="text-sm font-semibold">
                            Preço: R$ {payload[0].value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                          </p>
                          {payload[1] && (
                            <p className="text-xs text-muted-foreground">
                              Banda Superior: R$ {payload[1].value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                            </p>
                          )}
                          {payload[3] && (
                            <p className="text-xs text-muted-foreground">
                              Banda Inferior: R$ {payload[3].value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                            </p>
                          )}
                        </div>
                      );
                    ...
                    content={({ active, payload }) => {
                      ...
                      return (
                        <div className="bg-card border rounded-lg p-3 shadow-lg">
                          ...
                          <Badge 
                            variant={
                              rsiValue > 70 ? 'destructive' :
                              rsiValue < 30 ? 'success' : 'secondary'
                            }
                            className="mt-1"
                          >
                            {rsiValue > 70 ? 'Sobrecompra' : rsiValue < 30 ? 'Sobrevenda' : 'Neutro'}
                          </Badge>
                        </div>
                      );
                    }}
                    ...
                    content={({ active, payload }) => {
                      ...
                      return (
                        <div className="bg-card border rounded-lg p-3 shadow-lg">
                          ...
                          <p className="text-sm">MACD: {(payload[0].value as number)?.toFixed(2)}</p>
                          <p className="text-sm">Signal: {(payload[1].value as number)?.toFixed(2)}</p>
                          <p className="text-sm">Histogram: {(payload[2].value as number)?.toFixed(2)}</p>
                        </div>
                      );
                    }}
                  />
