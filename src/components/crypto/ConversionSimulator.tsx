...
export function ConversionSimulator() {
  ...
  const [bestMoment, setBestMoment] = useState<any>(null);
  ...
  const generateHistoricalData = () => {
    const days = 30;
    const baseRate = coinType === 'BTC' ? 350000 : coinType === 'ETH' ? 18000 : 5.5;
    const data = [];
    ...
    setBestMoment({
      ...
      currentRate,
      percentageFromMax,
      recommendation: percentageFromMax > -5 ? 'CONVERTER_AGORA' : 
                     percentageFromMax > -15 ? 'AGUARDAR' : 'EXCELENTE_MOMENTO',
    });
  };
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select value={coinType} onValueChange={(value: any) => setCoinType(value)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <Button onClick={() => { generateHistoricalData(); generateExchangeRates(); }} className="w-full">
            Atualizar Simulação
          </Button>
        </CardContent>
      </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-semibold">
                    R$ {bestMoment.currentRate.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </p>
                </div>
                <div>
                  <span className="text-muted-foreground">Taxa Máxima (30d):</span>
                  <p className="font-semibold">
                    R$ {bestMoment.maxRate.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </p>
                </div>
                <div>
                  <span className="text-muted-foreground">Diferença da Máxima:</span>
                  <p className={`font-semibold flex items-center gap-1 ${
                    bestMoment.percentageFromMax > 0 ? 'text-success' : 'text-destructive'
                  }`}>
            ...
            <LineChart data={historicalData}>
                  ...
                  return (
                    <div className="bg-card border rounded-lg p-3 shadow-lg">
                      <p className="text-xs text-muted-foreground mb-1">
                        {format(new Date(payload[0].payload.date), 'dd/MM/yyyy', { locale: ptBR })}
                      </p>
                      <p className="text-sm font-semibold">
                        R$ {payload[0].value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </p>
                      <p className={`text-xs ${
                        payload[0].payload.variation > 0 ? 'text-success' : 'text-destructive'
                      }`}>
              ...
              {bestMoment && (
                <ReferenceLine
                  ...
                  strokeDasharray="5 5"
                  label={{ value: 'Máxima', position: 'top', fill: 'hsl(var(--success))' }}
                />
              )}
              <Line
                type="monotone"
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-semibold">
                    R$ {(rate.rate * parseFloat(amount || '1')).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </p>
                </div>
                <div>
                  <p className="text-muted-foreground mb-1">Valor Líquido</p>
                  <p className={`font-semibold ${index === 0 ? 'text-success' : ''}`}>
                    R$ {rate.netAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </p>
                </div>
              </div>
              ...
              {index === 0 && exchangeRates.length > 1 && (
                <div className="mt-3 pt-3 border-t">
                  <p className="text-xs text-success">
                    💰 Economia de R$ {(rate.netAmount - exchangeRates[exchangeRates.length - 1].netAmount).toLocaleString('pt-BR', { minimumFractionDigits: 2 })} comparado à pior taxa
                  </p>
                </div>
              )}
            </div>
