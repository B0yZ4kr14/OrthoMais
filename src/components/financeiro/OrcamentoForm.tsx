...
export function OrcamentoForm({ onSubmit, onCancel, initialData }: OrcamentoFormProps) {
  ...
  const addItem = () => {
    const newItens = [...itens, { descricao: '', quantidade: 1, valor_unitario: 0 }];
    setItens(newItens);
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Select onValueChange={(value) => setValue('tipo_plano', value as any)} defaultValue={watch('tipo_plano')}>
            <SelectTrigger>
        ...
        <div className="space-y-2 md:col-span-2">
          ...
          <Input id="titulo" {...register('titulo')} placeholder="Ex: Tratamento Ortodôntico Completo" />
          {errors.titulo && <p className="text-sm text-destructive">{errors.titulo.message}</p>}
        </div>

        <div className="space-y-2 md:col-span-2">
          <Label htmlFor="descricao">Descrição</Label>
          <Textarea id="descricao" {...register('descricao')} placeholder="Descrição detalhada do orçamento" rows={3} />
        </div>

        <div className="space-y-2">
          <Label htmlFor="validade_dias">Validade (dias) *</Label>
          <Input id="validade_dias" type="number" {...register('validade_dias', { valueAsNumber: true })} />
        </div>

        <div className="space-y-2">
          <Label htmlFor="desconto_percentual">Desconto (%)</Label>
          <Input id="desconto_percentual" type="number" step="0.01" {...register('desconto_percentual', { valueAsNumber: true })} />
        </div>

        <div className="space-y-2">
          <Label htmlFor="desconto_valor">Desconto (R$)</Label>
          <Input id="desconto_valor" type="number" step="0.01" {...register('desconto_valor', { valueAsNumber: true })} />
        </div>
      </div>
      ...
      <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => updateItem(index, 'dente_codigo', e.target.value)}
                  placeholder="Ex: 11"
                />
              </div>
              ...
              <div className="flex items-end">
                <Button type="button" variant="destructive" size="sm" onClick={() => removeItem(index)} disabled={itens.length === 1}>
        ...
        {errors.itens && <p className="text-sm text-destructive">{errors.itens.message}</p>}
      </div>
      ...
      <div className="space-y-2">
        ...
        <Textarea id="observacoes" {...register('observacoes')} placeholder="Condições, garantias, etc." rows={3} />
      </div>
