...
export function useFontSize() {
  const [fontSize, setFontSize] = useLocalStorage<number>('ortho-font-size', DEFAULT_FONT_SIZE);

  useEffect(() => {
    // Clamp fontSize between min and max
    const clampedSize = Math.min(Math.max(fontSize, MIN_FONT_SIZE), MAX_FONT_SIZE);
