...
describe('ModulesConfig', () => {
  describe('MODULES_CONFIG', () => {
    ...
    it('não deve ter dependências circulares', () => {
      const checkCircular = (moduleKey: string, visited: Set<string> = new Set()): boolean => {
  ...
  describe('hasAllDependencies', () => {
    it('deve retornar true se todas as dependências estão ativas', () => {
      const result = hasAllDependencies('SPLIT_PAGAMENTO', ['FINANCEIRO', 'PEP']);
  ...
  describe('groupModulesByCategory', () => {
    ...
    it('deve agrupar módulos por categoria', () => {
      ...
      expect(grouped.find(g => g.name === 'Gestão Financeira')?.modules.length).toBe(1);
      expect(grouped.find(g => g.name === 'Atendimento Clínico')?.modules.length).toBe(1);
    });
