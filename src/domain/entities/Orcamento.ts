...
export class Orcamento {
   ...
   */
  static create(
    props: Omit<OrcamentoProps, 'id' | 'numeroOrcamento' | 'createdAt' | 'updatedAt' | 'status' | 'dataExpiracao'>
  ): Orcamento {
  ...
  enviarParaAprovacao(): void {
    if (!this.podeSerEnviado()) {
      throw new Error('Apenas orçamentos em RASCUNHO e não expirados podem ser enviados');
    }
  ...
  aprovar(aprovadoPor: string): void {
    if (!this.podeSerAprovado()) {
      throw new Error('Apenas orçamentos PENDENTES e não expirados podem ser aprovados');
    }
  ...
  marcarExpirado(): void {
    if (this.props.status !== 'PENDENTE') {
      throw new Error('Apenas orçamentos PENDENTES podem ser marcados como expirados');
    }
