...
export const fidelidadePontosSchema = z.object({
  ...
  nivel: z.enum(['BRONZE', 'PRATA', 'OURO', 'PLATINA', 'DIAMANTE']).default('BRONZE'),
});
...
export const fidelidadeTransacaoSchema = z.object({
  ...
  clinic_id: z.string().uuid(),
  patient_id: z.string().uuid(),
  tipo: z.enum(['GANHO_CONSULTA', 'GANHO_PAGAMENTO', 'GANHO_INDICACAO', 'RESGATE', 'EXPIRACAO']),
  pontos: z.number(),
  descricao: z.string(),
...
export const fidelidadeRecompensaSchema = z.object({
  ...
  tipo: z.enum(['DESCONTO_PERCENTUAL', 'DESCONTO_VALOR', 'PROCEDIMENTO_GRATIS', 'BRINDE']),
...
export const fidelidadeIndicacaoSchema = z.object({
  ...
  status: z.enum(['PENDENTE', 'AGENDADO', 'COMPARECEU', 'NAO_COMPARECEU']).default('PENDENTE'),
