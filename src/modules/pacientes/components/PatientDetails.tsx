...
import { Edit, Calendar, FileText, Phone, Mail, MapPin, CreditCard, User } from 'lucide-react';
      ...
      <Tabs defaultValue="dados" className="w-full">
        ...
        <TabsContent value="dados" className="space-y-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-sm text-muted-foreground">CEP: {patient.endereco.cep}</p>
            </CardContent>
          </Card>
            ...
            <CardContent>
              {patient.convenio.temConvenio ? (
                <div className="space-y-2">
                  <div>
                    <p className="text-sm text-muted-foreground">Nome do Convênio</p>
                    <p className="font-medium">{patient.convenio.nomeConvenio}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Número da Carteira</p>
                    <p className="font-medium">{patient.convenio.numeroCarteira}</p>
                  </div>
                  {patient.convenio.validade && (
                    <div>
                      <p className="text-sm text-muted-foreground">Validade</p>
                      <p className="font-medium">
                        {new Date(patient.convenio.validade).toLocaleDateString('pt-BR')}
                      </p>
                    </div>
                  )}
                </div>
        ...
        <TabsContent value="consultas" className="space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-semibold">Consultas Realizadas e Agendadas</h3>
            <Button size="sm">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <CardTitle className="text-base">
                      Atendimento - {new Date(prontuario.data).toLocaleDateString('pt-BR')}
                    </CardTitle>
                    <p className="text-sm text-muted-foreground">{prontuario.dentista}</p>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                {prontuario.anamnese && (
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{prontuario.anamnese}</p>
                  </div>
                )}
                {prontuario.diagnostico && (
                  <div>
                    <p className="font-semibold text-sm mb-1">Diagnóstico</p>
                    <p className="text-sm text-muted-foreground">{prontuario.diagnostico}</p>
                  </div>
                )}
                {prontuario.tratamento && (
                  <div>
                    <p className="font-semibold text-sm mb-1">Tratamento Realizado</p>
                    <p className="text-sm text-muted-foreground">{prontuario.tratamento}</p>
                  </div>
                )}
                {prontuario.prescricao && (
                  <div>
                    <p className="font-semibold text-sm mb-1">Prescrição / Orientações</p>
                    <p className="text-sm text-muted-foreground">{prontuario.prescricao}</p>
                  </div>
                )}
                {prontuario.observacoes && (
                  <div>
                    <p className="font-semibold text-sm mb-1">Observações</p>
                    <p className="text-sm text-muted-foreground">{prontuario.observacoes}</p>
                  </div>
                )}
              </CardContent>
            </Card>
