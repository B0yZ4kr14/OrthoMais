...
import { AlertCircle, CheckCircle, Clock, XCircle, Lightbulb } from 'lucide-react';
...
export function AnaliseDetailsDialog({ analise, open, onOpenChange }: AnaliseDetailsDialogProps) {
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6" depth="normal">
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                <h3 className="text-sm font-medium text-muted-foreground mb-1">Data da Análise</h3>
                <p className="text-lg">{new Date(analise.created_at).toLocaleString('pt-BR')}</p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-muted-foreground mb-1">Tipo de Radiografia</h3>
              ...
              <div>
                ...
                <div className={`flex items-center gap-2 ${getStatusColor(analise.status_analise)}`}>
                  {getStatusIcon(analise.status_analise)}
                  <span className="font-medium capitalize">{analise.status_analise}</span>
                </div>
              </div>
            </div>
          </Card>
          ...
          {analise.confidence_score && analise.confidence_score > 0 && (
            <Card className="p-6 bg-primary/5 border-primary/20" depth="subtle">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold mb-1">Confiança da IA</h3>
          ...
          {problemas.length > 0 && (
            <Card className="p-6" depth="normal">
              ...
              <div className="space-y-4">
                {problemas.map((problema: any, index: number) => (
                  <div key={index} className="border rounded-lg p-4 hover:bg-accent/50 transition-colors">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <h4 className="font-semibold">{problema.tipo || 'Problema Dentário'}</h4>
                        <p className="text-sm text-muted-foreground mt-1">
                          {problema.localizacao || 'Localização não especificada'}
                        </p>
                      </div>
                      <Badge variant={getSeverityColor(problema.severidade) as any}>
              ...
              <div className="space-y-4">
                {sugestoes.map((sugestao: any, index: number) => (
                  <div key={index} className="border rounded-lg p-4 bg-background">
                    <h4 className="font-semibold mb-2">{sugestao.tratamento || 'Tratamento Recomendado'}</h4>
          ...
          {analise.observacoes_dentista && (
            <Card className="p-6" depth="normal">
              <h3 className="text-lg font-semibold mb-4">Observações do Dentista</h3>
              <p className="text-sm whitespace-pre-wrap">{analise.observacoes_dentista}</p>
