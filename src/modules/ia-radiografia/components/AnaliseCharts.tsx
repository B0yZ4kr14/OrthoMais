...
export function AnaliseCharts({ analises }: AnaliseChartsProps) {
  ...
  const analisesPorMes = useMemo(() => {
    const meses = new Map<string, { mes: string; total: number; concluidas: number; problemas: number }>();
    ...
    analises.forEach((analise) => {
      ...
      if (!meses.has(mesAno)) {
        meses.set(mesAno, { mes: mesAno, total: 0, concluidas: 0, problemas: 0 });
      }
  ...
  const analisesPorStatus = useMemo(() => {
    ...
    analises.forEach((analise) => {
      status.set(analise.status_analise, (status.get(analise.status_analise) || 0) + 1);
    });
  ...
  const precisaoAoLongoTempo = useMemo(() => {
    const meses = new Map<string, { mes: string; precisao: number; count: number }>();
      ...
      <Tabs defaultValue="temporal" className="space-y-4">
        ...
        <TabsContent value="temporal" className="space-y-4">
          <div className="space-y-2">
            <h3 className="text-sm font-semibold">Análises e Problemas Detectados (Últimos 6 Meses)</h3>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={analisesPorMes}>
                ...
                />
                <Legend />
                <Bar dataKey="total" name="Total de Análises" fill={CHART_COLORS.primary} radius={[8, 8, 0, 0]} />
                <Bar dataKey="concluidas" name="Concluídas" fill={CHART_COLORS.success} radius={[8, 8, 0, 0]} />
                <Bar dataKey="problemas" name="Problemas Detectados" fill={CHART_COLORS.warning} radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </TabsContent>
        ...
        <TabsContent value="tipos" className="space-y-4">
          <div className="space-y-2">
            <h3 className="text-sm font-semibold">Distribuição por Tipo de Radiografia</h3>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
        ...
        <TabsContent value="precisao" className="space-y-4">
          <div className="space-y-2">
            <h3 className="text-sm font-semibold">Evolução da Precisão da IA (Últimos 6 Meses)</h3>
            <ResponsiveContainer width="100%" height={300}>
