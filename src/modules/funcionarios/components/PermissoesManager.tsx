...
export function PermissoesManager({ permissoes, cargo, onChange }: PermissoesManagerProps) {
  const [currentPermissoes, setCurrentPermissoes] = useState<Permissoes>(permissoes);
  ...
  const handleToggleTodos = (modulo: string, marcar: boolean) => {
    const permissions = permissoesDisponiveis[modulo as keyof typeof permissoesDisponiveis].permissions;
    const updated: Permissoes = {
      ...currentPermissoes,
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {Object.entries(permissoesDisponiveis).map(([modulo, config]) => {
          const moduloPermissoes = currentPermissoes[modulo] || [];
          const todasMarcadas = config.permissions.every(p => moduloPermissoes.includes(p));
          const algumasMarcadas = config.permissions.some(p => moduloPermissoes.includes(p));
                ...
                {config.permissions.map((acao) => (
                  <div key={acao} className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={() => handleTogglePermissao(modulo, acao)}
                    />
                    <label
                      htmlFor={`${modulo}-${acao}`}
