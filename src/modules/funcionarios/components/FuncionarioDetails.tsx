import { Funcionario, diasSemana, permissoesDisponiveis } from '../types/funcionario.types';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Separator } from '@/components/ui/separator';
import { Edit, User, Phone, MapPin, Briefcase, Calendar, Clock, DollarSign, Shield } from 'lucide-react';
...
export function FuncionarioDetails({ funcionario, onEdit, onClose }: FuncionarioDetailsProps) {
  ...
  const getPermissoesAtivas = () => {
    const permissoesAtivas: { modulo: string; label: string; acoes: string[] }[] = [];
    
    Object.entries(funcionario.permissoes).forEach(([modulo, acoes]) => {
      if (acoes.length > 0) {
        const config = permissoesDisponiveis[modulo as keyof typeof permissoesDisponiveis];
        if (config) {
      ...
      <div className="flex items-start justify-between">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <Badge variant={getStatusColor(funcionario.status)}>{funcionario.status}</Badge>
          ...
          <p className="text-sm text-muted-foreground">
            Cadastrado em {format(parseISO(funcionario.createdAt!), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div>
              ...
              <p className="font-medium">
                {funcionario.sexo === 'M' ? 'Masculino' : funcionario.sexo === 'F' ? 'Feminino' : 'Outro'}
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-sm text-muted-foreground">CEP: {funcionario.endereco.cep}</p>
          </CardContent>
        </Card>
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent className="space-y-3">
            ...
            <div>
              ...
              <p className="font-medium">
                {funcionario.horarioTrabalho.inicio} às {funcionario.horarioTrabalho.fim}
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent>
          {getPermissoesAtivas().length === 0 ? (
            <p className="text-sm text-muted-foreground">Nenhuma permissão concedida</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {getPermissoesAtivas().map(({ modulo, label, acoes }) => (
                <div key={modulo} className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {acoes.map((acao) => (
                      <Badge key={acao} variant="outline" className="text-xs capitalize">
                        {acao}
                      </Badge>
                    ))}
                  </div>
        ...
        <CardContent className="space-y-2 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium">
              {format(parseISO(funcionario.createdAt!), 'dd/MM/yyyy HH:mm', { locale: ptBR })}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">Última atualização:</span>
            <span className="font-medium">
              {format(parseISO(funcionario.updatedAt!), 'dd/MM/yyyy HH:mm', { locale: ptBR })}
            </span>
          </div>
        </CardContent>
      </Card>
