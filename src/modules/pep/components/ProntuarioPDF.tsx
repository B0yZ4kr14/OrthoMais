...
export function ProntuarioPDF({ prontuarioId, patientName }: ProntuarioPDFProps) {
  const generatePDF = () => {
    const doc = new jsPDF();
    ...
    doc.setFontSize(10);
    doc.setTextColor(60);
    doc.text('Anamnese, diagnósticos e histórico médico completo do paciente.', margin, yPos);
    ...
    doc.setFontSize(10);
    doc.setTextColor(60);
    doc.text('Histórico de procedimentos e tratamentos odontológicos.', margin, yPos);
    ...
    doc.setFontSize(8);
    doc.setTextColor(100);
    doc.text('Este documento é uma representação digital do prontuário eletrônico.', pageWidth / 2, footerY, { align: 'center' });
    doc.text(`Gerado em ${new Date().toLocaleString('pt-BR')} pelo sistema ORTHO +`, pageWidth / 2, footerY + 4, { align: 'center' });

    // Save PDF
    doc.save(`prontuario_${patientName.replace(/\s+/g, '_')}_${new Date().getTime()}.pdf`);
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Exporte o prontuário completo com histórico, tratamentos e prescrições
          </p>
        </div>
      </div>
