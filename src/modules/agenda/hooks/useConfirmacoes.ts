...
export function useConfirmacoes(agendamentoId?: string) {
  ...
  const sendConfirmacaoWhatsAppUseCase = container.resolve<SendConfirmacaoWhatsAppUseCase>(
  ...
  const fetchConfirmacao = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const result = await confirmacaoRepository.findByAgendamentoId(agendamentoId);
   ...
   */
  const sendWhatsApp = useCallback(async (data: {
   ...
   */
  const confirmarManualmente = useCallback(async (confirmacaoId: string) => {
    setIsLoading(true);
    try {
