...
export function useAgendamentos(clinicId: string, startDate?: Date, endDate?: Date, dentistId?: string) {
  ...
  const getAgendamentosByDateRangeUseCase = container.resolve<GetAgendamentosByDateRangeUseCase>(
  ...
  }, [clinicId, startDate, endDate, dentistId, agendamentoRepository, getAgendamentosByDateRangeUseCase, toast]);
   ...
   * Cancela agendamento
   */
  const cancelAgendamento = useCallback(async (agendamentoId: string) => {
    setIsLoading(true);
    try {
   ...
   * Inicia atendimento
   */
  const iniciarAtendimento = useCallback(async (agendamentoId: string) => {
    setIsLoading(true);
    try {
      const agendamento = await agendamentoRepository.findById(agendamentoId);
      if (!agendamento) {
   ...
   * Conclui atendimento
   */
  const concluirAtendimento = useCallback(async (agendamentoId: string) => {
    setIsLoading(true);
    try {
      const agendamento = await agendamentoRepository.findById(agendamentoId);
      if (!agendamento) {
   ...
   * Marca falta
   */
  const marcarFalta = useCallback(async (agendamentoId: string) => {
    setIsLoading(true);
    try {
      const agendamento = await agendamentoRepository.findById(agendamentoId);
      if (!agendamento) {
