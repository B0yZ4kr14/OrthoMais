...
export class CreateAppointmentUseCase {
  ...
  async execute(input: CreateAppointmentInput): Promise<Appointment> {
    ...
    const blockedTimes = await this.blockedTimeRepository.findByDentistAndDateRange(
      input.dentistId,
      input.scheduledDatetime,
      new Date(input.scheduledDatetime.getTime() + input.durationMinutes * 60000)
    );
    ...
    const endDatetime = new Date(input.scheduledDatetime.getTime() + input.durationMinutes * 60000);
