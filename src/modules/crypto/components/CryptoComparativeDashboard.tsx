...
export function CryptoComparativeDashboard({ transactions }: CryptoComparativeDashboardProps) {
  ...
  const cryptoFeePercentage =
    cryptoStats.totalBRL > 0 ? (cryptoStats.totalFees / cryptoStats.totalBRL) * 100 : 0;
  ...
  const comparisonData = [
    ...
    },
    {
      ...
      netAmount: cryptoStats.totalBRL - (cryptoStats.totalBRL * TRADITIONAL_FEES.PIX) / 100,
    ...
    },
    {
      ...
      netAmount: cryptoStats.totalBRL - (cryptoStats.totalBRL * TRADITIONAL_FEES.DEBIT_CARD) / 100,
    ...
    },
    {
      ...
      netAmount: cryptoStats.totalBRL - (cryptoStats.totalBRL * TRADITIONAL_FEES.CREDIT_CARD) / 100,
    ...
    },
    {
      ...
      netAmount: cryptoStats.totalBRL - (cryptoStats.totalBRL * TRADITIONAL_FEES.BOLETO) / 100,
  ...
  const savingsData = comparisonData.slice(1).map((method) => ({
    ...
    savingsPercentage: ((method.fee - cryptoStats.totalFees) / method.fee) * 100,
  }));

  const totalSavings = savingsData.reduce((sum, item) => sum + item.savings, 0);
  const avgSavingsPercentage = savingsData.reduce((sum, item) => sum + item.savingsPercentage, 0) / savingsData.length;
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center text-muted-foreground">
          ...
          <p className="text-sm mt-2">Confirme pagamentos para visualizar análise comparativa.</p>
        </CardContent>
      </Card>
    );
  }
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                R$ {cryptoStats.totalFees.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
              </p>
            </div>
        ...
        <Card variant="metric" depth="normal" className="p-6 border-l-green-500">
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground mt-1">vs métodos tradicionais</p>
            </div>
        ...
        <Card variant="metric" depth="normal" className="p-6 border-l-blue-500">
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-2xl font-bold text-blue-500">{avgSavingsPercentage.toFixed(1)}%</p>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-2xl font-bold text-purple-500">
                R$ {cryptoStats.netAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
              </p>
              <p className="text-xs text-muted-foreground mt-1">recebido após taxas</p>
            </div>
      ...
      {totalSavings > 100 && (
        <Alert className="bg-green-500/10 border-green-500/50">
          ...
          <AlertDescription className="text-green-700 dark:text-green-300">
            ...
            <strong>R$ {totalSavings.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</strong>{' '}
            em taxas usando pagamentos em criptomoedas comparado a métodos tradicionais.
          </AlertDescription>
        </Alert>
      )}
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={comparisonData}>
                ...
                <YAxis
                  label={{ value: 'Taxa (R$)', angle: -90, position: 'insideLeft' }}
                ...
                />
                <Legend />
                <Bar dataKey="fee" name="Taxa Cobrada" fill="#f97316" radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={savingsData}>
              ...
              <XAxis dataKey="method" />
              <YAxis
                label={{ value: 'Economia (R$)', angle: -90, position: 'insideLeft' }}
                tickFormatter={(value) => `R$ ${value.toFixed(0)}`}
              />
              <Tooltip
                formatter={(value: any) =>
              ...
              />
              <Legend />
              <Bar dataKey="savings" name="Economia em Taxas" fill="#10b981" radius={[8, 8, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
                ...
                {comparisonData.map((item) => {
                  ...
                  return (
                    <tr key={item.method} className="border-b hover:bg-muted/50">
                      ...
                      <td className="text-right py-3 px-4">{item.feePercentage.toFixed(2)}%</td>
                      <td className="text-right py-3 px-4">
                        R$ {item.fee.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </td>
                      <td className="text-right py-3 px-4">
