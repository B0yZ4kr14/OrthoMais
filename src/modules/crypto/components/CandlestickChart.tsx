  ...
  return (
    <Card className="p-6" depth="normal">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-semibold">Gráfico Candlestick - {coinType}</h3>
          <p className="text-sm text-muted-foreground">Análise técnica avançada com padrões intraday</p>
        </div>
                ...
                return (
                  <Card className="p-3 shadow-lg">
                    <div className="space-y-1 text-sm">
                      <div className="font-semibold">{new Date(data.time).toLocaleString('pt-BR')}</div>
                      <div className="grid grid-cols-2 gap-2">
                        <span className="text-muted-foreground">Abertura:</span>
                        <span className="font-mono">R$ {data.open.toFixed(2)}</span>
                        <span className="text-muted-foreground">Máxima:</span>
                        <span className="font-mono text-success">R$ {data.high.toFixed(2)}</span>
                        <span className="text-muted-foreground">Mínima:</span>
                        <span className="font-mono text-destructive">R$ {data.low.toFixed(2)}</span>
                        <span className="text-muted-foreground">Fechamento:</span>
                        <span className={`font-mono ${data.isGreen ? 'text-success' : 'text-destructive'}`}>
                          R$ {data.close.toFixed(2)}
                        </span>
                        <span className="text-muted-foreground">Volume:</span>
                        <span className="font-mono">{data.volume.toLocaleString('pt-BR')}</span>
                      </div>
                    </div>
                  </Card>
                );
      ...
      <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
        ...
        <div>
          ...
          <span className="ml-2 font-semibold">
            {zoomDomain ? `${Math.round((zoomDomain[1] - zoomDomain[0]) / data.length * 100)}%` : '100%'}
          </span>
        </div>
      </div>
    </Card>
