...
interface ContasPagarListProps {
  contas: ContaPagar[];
  loading: boolean;
  onPagar: (contaId: string, valorPago?: number, formaPagamento?: string) => Promise<void>;
}

export function ContasPagarList({ contas, loading, onPagar }: ContasPagarListProps) {
  if (loading) {
  ...
  const getStatusBadge = (conta: ContaPagar) => {
    if (conta.isPaga()) {
      return <Badge variant="default" className="bg-success"><CheckCircle2 className="mr-1 h-3 w-3" />Paga</Badge>;
    }
    if (conta.isVencida()) {
              ...
              <div className="space-y-1">
                ...
                {conta.dataPagamento && (
                  <p className="text-sm text-success">
                    Pago em: {format(conta.dataPagamento, "dd/MM/yyyy", { locale: ptBR })}
                  </p>
                )}
              </div>
              {conta.isPendente() && (
