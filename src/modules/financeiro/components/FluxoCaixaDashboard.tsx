...
export function FluxoCaixaDashboard({ data, loading }: FluxoCaixaDashboardProps) {
  if (loading) {
    ...
    <div className="space-y-4">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-success">{formatCurrency(data.totalReceber)}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-destructive">{formatCurrency(data.totalPagar)}</div>
            <p className="text-xs text-muted-foreground mt-1">
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-success">{formatCurrency(data.totalRecebido)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-muted-foreground">Saldo a Receber</span>
              <span className="font-medium">{formatCurrency(data.saldoReceber)}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-destructive">{formatCurrency(data.totalPago)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-muted-foreground">Saldo a Pagar</span>
              <span className="font-medium">{formatCurrency(data.saldoPagar)}</span>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent className="space-y-2">
            ...
            {data.caixaAtual.valorInicial !== undefined && (
              <div className="flex justify-between">
                ...
                <span className="font-medium">{formatCurrency(data.caixaAtual.valorInicial)}</span>
              </div>
            )}
          </CardContent>
        </Card>
