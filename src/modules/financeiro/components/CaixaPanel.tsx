...
interface CaixaPanelProps {
  ...
  onFecharCaixa: (valorFinal: number, valorEsperado: number, observacoes?: string) => Promise<void>;
  onRegistrarSangria: (valor: number, motivo: string, userId: string, horarioRisco?: string) => Promise<void>;
  userId: string;
}
...
export function CaixaPanel({
  ...
  if (loading) {
    return <div className="text-center py-8">Carregando informações do caixa...</div>;
  }
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {caixaAberto ? 'Caixa aberto para movimentações' : 'Caixa fechado'}
              </CardDescription>
            </div>
                ...
                <>
                  <Dialog open={isSangriaDialogOpen} onOpenChange={setIsSangriaDialogOpen}>
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4">
                        ...
                        <Button onClick={handleRegistrarSangria} className="w-full">
                          Confirmar Sangria
                        </Button>
                      </div>
                    </DialogContent>
                  ...
                  <Dialog open={isFecharDialogOpen} onOpenChange={setIsFecharDialogOpen}>
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4">
                        <div>
                          <Label htmlFor="valorFinal">Valor Final (Contado)</Label>
                          <Input
                            id="valorFinal"
                            type="number"
                            step="0.01"
                        ...
                        <div>
                          <Label htmlFor="valorEsperado">Valor Esperado (Sistema)</Label>
                          <Input
                            id="valorEsperado"
                            type="number"
                            step="0.01"
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-semibold">
                  {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(caixaAtual.valorInicial)}
                </span>
              </div>
              {caixaAtual.valorFinal !== undefined && (
                <>
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">
                      {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(caixaAtual.valorFinal)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Diferença:</span>
                    <span className={`font-semibold ${caixaAtual.calcularDiferenca() >= 0 ? 'text-success' : 'text-destructive'}`}>
                      {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(caixaAtual.calcularDiferenca())}
                    </span>
                  </div>
                </>
              )}
