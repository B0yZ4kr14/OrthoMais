...
interface ContasReceberListProps {
  contas: ContaReceber[];
  loading: boolean;
  onReceber: (contaId: string, valorPago?: number, formaPagamento?: string) => Promise<void>;
}

export function ContasReceberList({ contas, loading, onReceber }: ContasReceberListProps) {
  if (loading) {
  ...
  const getStatusBadge = (conta: ContaReceber) => {
    if (conta.isRecebida()) {
      return <Badge variant="default" className="bg-success"><CheckCircle2 className="mr-1 h-3 w-3" />Recebida</Badge>;
    }
    if (conta.isVencida()) {
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Vencimento: {format(conta.dataVencimento, "dd/MM/yyyy", { locale: ptBR })}
                </p>
                {conta.dataPagamento && (
                  <p className="text-sm text-success">
                    Recebido em: {format(conta.dataPagamento, "dd/MM/yyyy", { locale: ptBR })}
                  </p>
                )}
              </div>
              {conta.isPendente() && (
