...
export class GetCashFlowUseCase {
  ...
  async execute(dto: GetCashFlowDTO): Promise<CashFlowResult> {
    const [totalReceitas, totalDespesas] = await Promise.all([
      this.transactionRepository.getTotalByPeriod(dto.clinicId, dto.period, 'RECEITA'),
      this.transactionRepository.getTotalByPeriod(dto.clinicId, dto.period, 'DESPESA'),
    ]);

    const pendingTransactions = await this.transactionRepository.getPendingTransactions(dto.clinicId);
