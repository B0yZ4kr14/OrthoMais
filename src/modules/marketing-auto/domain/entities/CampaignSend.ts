...
export class CampaignSend {
  ...
  private validate(): void {
    ...
    if (!this.props.recipientName || this.props.recipientName.trim().length === 0) {
      throw new Error('Nome do destinatário é obrigatório');
    }

    if (!this.props.recipientContact || this.props.recipientContact.trim().length === 0) {
  ...
  get recipientContact(): string { return this.props.recipientContact; }
  get messageContent(): string | undefined { return this.props.messageContent; }
  get status(): CampaignSendStatus { return this.props.status; }
  ...
  get clickedAt(): Date | undefined { return this.props.clickedAt; }
  get convertedAt(): Date | undefined { return this.props.convertedAt; }
  ...
  isSent(): boolean {
    return ['ENVIADO', 'ENTREGUE', 'ABERTO', 'CLICADO', 'CONVERTIDO'].includes(this.props.status);
  }

  isDelivered(): boolean {
    return ['ENTREGUE', 'ABERTO', 'CLICADO', 'CONVERTIDO'].includes(this.props.status);
  }
  ...
  markAsSent(): void {
    if (!this.isScheduled()) {
      throw new Error('Envio só pode ser marcado como enviado se estiver agendado');
    }
  ...
  markAsConverted(): void {
    if (!this.isClicked() && !this.isOpened()) {
      throw new Error('Envio só pode ser marcado como convertido se foi aberto ou clicado');
    }
  ...
  reschedule(newDate: Date): void {
    if (!this.hasError() && !this.isScheduled()) {
      throw new Error('Apenas envios com erro ou agendados podem ser reagendados');
    }
